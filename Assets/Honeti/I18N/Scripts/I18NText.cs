    ...
    public class I18NText : MonoBehaviour
    {
        ...
        private void _updateTranslation()
        {
            if (_text)
            {
                ...
                _text.text = ManagerRoot.I18N.getValue(_key, _params);
            }
        }
        ...
        private void _init()
        {
            ...
            if(_text == null)
            {
                Debug.LogError("I18NText | Fail GetComponent<TMP_Text>() ");
                return;
            }
            _key = _text.text;
            _initialized = true;

            // if (ManagerRoot.I18N.useCustomFonts)
            // {
            //     _changeFont(ManagerRoot.I18N.customFont);
            // }
            ...
            if (!_key.StartsWith("^"))
            {
                Debug.LogWarning(string.Format("I18NText | {0}: Translation key was not found! Found {1}", this, _key));
                _isValidKey = false;
            }
            else
            {
            ...
            if (!_text)
            {
                Debug.LogWarning(string.Format("I18NText | {0}: Text component was not found!", this));
            }
        }
