...
public class UIDefeatPopup : UIPopup
{
	...
	UIPopupButton _btnRestart, _btnToTitle;
	...
	protected override void Init()
	{
		...
		{
			Image dieImage = Get<Image>((int)Images.DieImage);
			dieImage.enabled = true;
		}
		else{
			Image dieImageCute = Get<Image>((int)Images.DieImageCute);
			dieImageCute.enabled = true;
		}
	...
	private void OnEnable() 
	{
		...
		eventSystem.SetSelectedGameObject(_btnRestart.gameObject);
	}
	...
	void SetMenuPanel() // todo; Grid로 변경하고 Destroy 해주기
	{
		...
		_btnRestart.SetButtonText("^text_panel_defeat_btn_Restart");
		...
		_btnToTitle.SetButtonText("^text_panel_defeat_btn_Totitle");
	...
	{
		Debug.Log("UIDefeatPopup | SetUnitGridPanel |");
	...
	{
		Debug.Log("UIDefeatPopup | MakeUnitContainer | enter");
		...
		Debug.Log("UIDefeatPopup | MakeUnitContainer | before switch");
		switch (unitType_)
		{
	...
	{
		Debug.Log("UIDefeatPopup | '타이틀로' 버튼 클릭 ");
		GameManager.Instance.DefeatGame();
	...
	// 	//unitConta.SetNameText($"{unitType_}");
